...
export async function POST(req: NextRequest) {
  ...
  if (body.topicSlug) {
    const result = await revisionService.startSessionBySlug(body.topicSlug);
    if (!result.success) return NextResponse.json({ error: result.error, code: result.code }, { status: 400 });
    return NextResponse.json(result.data, { status: 201 });
  }

  if (body.topicId) {
    const result = await revisionService.startSession(body.topicId);
    if (!result.success) return NextResponse.json({ error: result.error, code: result.code }, { status: 400 });
    return NextResponse.json(result.data, { status: 201 });
  }
...
export async function GET(req: NextRequest) {
  ...
  const sessions = await revisionService.getRecentSessions(limit);
